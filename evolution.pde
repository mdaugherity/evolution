...
int countdown; // extra frames before restart
...
void setup() {
  ...
  b.reset();
  countdown = -1;  // negative while running
...
void draw() {
  ...
  textFont(gfont);
  String s;
  if (countdown<0) s = "Generation " + generation;
  else s = "Breeding Generation " + generation;
  text(s, g[0].statusx-3, g[0].statusy-17);

  b.update();
  b.draw();

  if (NUMALIVE==0 && countdown<0) countdown = 150;
  
  if (NUMALIVE==0 && countdown>0) countdown--;

  if (NUMALIVE==0 && countdown==0) {
